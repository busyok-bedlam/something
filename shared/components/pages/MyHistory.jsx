...
export default class MyHistory extends Component {
    ...
    async _onSelect(selected) {
        try {
            ...
            LoadingScreen.open();
            await redBlackGameActions.getHistory(0, selected.value);
        } catch (error) {
            console.error(error);
    ...
    async componentDidMount() {
        try {
            const {redBlackGameActions} = this.props;
            LoadingScreen.open();
            await redBlackGameActions.getHistory();
    ...
    __onScrollBottom() {
        ...
        if (offset > height - 100) {
            ...
            if (!redBlackGame.isLoading && !redBlackGame.historyLastLoaded) {
                redBlackGameActions.getHistory(redBlackGame.historyPage + 1, selected);
            }
        }
    }
    ...
    render() {
        return (
            <div className="my-history row">
                <div className="col-md-8">
                    <h2 className="main-header">My History</h2>
